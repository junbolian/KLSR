function svm_cervical_main()
% svm_cervical_main.m
% Single-dataset runner: cervicalcancerbehaviorrisk.csv
...
%
% Cervical (verified by Python):
%   - label: ca_cervix (0/1), distribution 0:51, 1:21
%   - all numeric, no NaN, no ID columns
%   - features = 19
...
[X_all, y_all] = load_cervical_strict(fpath);

fprintf('DBG cervical: n=%d, d=%d, label_unique=%d\n', ...
...
for r = 1:runs
    ...
    % ---- safety fill NaN by TRAIN median (should do nothing here) ----
...
out_csv = fullfile(OUT_DIR, 'cervical_result.csv');
...
function [X, y] = load_cervical_strict(fpath)
% Loader for cervicalcancerbehaviorrisk.csv
% - label: ca_cervix (0/1)
% - all numeric, no drops required

T = readtable(fpath, 'PreserveVariableNames', true);

if ~ismember('ca_cervix', T.Properties.VariableNames)
    error('Label column "ca_cervix" not found. Columns=%s', strjoin(T.Properties.VariableNames, ', '));
end

y = double(T.ca_cervix(:));
T.ca_cervix = [];
