function svm_seeds_main()
% svm_seeds_main.m
% Single-dataset runner: seeds.csv
% - Compare 4 metaheuristics + their KLSR-augmented versions (8 total)
% - NO cross validation
% - Outer holdout: train/test (test_ratio=0.50)
% - Inner holdout within train: train/val (val_in_train=0.25)
% - N=30, Max_iter=30, runs=10
%
% Verified by your Python inspector:
%   - n=199, d=8
%   - label: Type (3 classes: 1/2/3)
%   - all numeric, no missing
% NOTE:
%   - This is multi-class => use fitcecoc with SVM template (RBF).
...
[X_all, y_all] = load_seeds_strict(fpath);

fprintf('DBG seeds: n=%d, d=%d, label_unique=%d\n', ...
...
for r = 1:runs
    ...
    fobj = @(x) svm_val_error_multiclass(x, X_tr, y_tr, X_val, y_val);
    ...
    for ai = 1:numel(algs)
        ...
        holdout_err(r, ai) = svm_test_error_multiclass(best_x, X_train_full, y_train_full, X_test, y_test);
    end
    fprintf('\n');
end
...
out_csv = fullfile(OUT_DIR, 'seeds_result.csv');
...
function err = svm_val_error_multiclass(x, X_tr, y_tr, X_val, y_val)
...
try
    ...
    mdl = fitcecoc(Z_tr, y_tr, 'Learners', t, 'Coding', 'onevsall');
...
function err = svm_test_error_multiclass(x, X_train, y_train, X_test, y_test)
...
try
    ...
    mdl = fitcecoc(Z_tr, y_train, 'Learners', t, 'Coding', 'onevsall');
...
function [X, y] = load_seeds_strict(fpath)
% Strict loader using column name "Type" as label (3-class)
try
    opts = detectImportOptions(fpath);
    opts = setvaropts(opts, opts.VariableNames, 'Whitespace', 'preserve');
...
j = find(lower(vn) == "type", 1);
if isempty(j)
    error("Label column 'Type' not found. Columns=%s", strjoin(vn, ", "));
end
