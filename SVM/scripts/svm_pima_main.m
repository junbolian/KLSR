function svm_pima_main()
% svm_pima_main.m
% Single-dataset runner: pima-indians-diabetes.csv
...
%
% Pima (verified by Python):
%   - label: Outcome (0/1), distribution 0:500, 1:268
%   - all numeric, no NaN
%
% IMPORTANT: Pima common convention:
%   Some feature columns use 0 to represent missing (not true zeros).
%   We treat 0 as missing (NaN) for: Glucose, BloodPressure, SkinThickness, Insulin, BMI
%   Then fill missing using TRAIN median within each split (to avoid leakage).
...
[X_all, y_all] = load_pima_strict(fpath);

fprintf('DBG pima: n=%d, d=%d, label_unique=%d\n', ...
...
for r = 1:runs
    ...
    % ---- fill missing using TRAIN median (includes 0-missing we converted to NaN) ----
...
out_csv = fullfile(OUT_DIR, 'pima_result.csv');
...
function [X, y] = load_pima_strict(fpath)
% Loader for pima-indians-diabetes.csv
% - label: Outcome (0/1)
% - treat 0 as missing for specific columns:
%   Glucose, BloodPressure, SkinThickness, Insulin, BMI
%   (convert zeros to NaN)
% - X returned as double; y as double column vector

T = readtable(fpath, 'PreserveVariableNames', true);

if ~ismember('Outcome', T.Properties.VariableNames)
    error('Label column "Outcome" not found. Columns=%s', strjoin(T.Properties.VariableNames, ', '));
end

y = double(T.Outcome(:));
T.Outcome = [];

% convert table to array but keep names for 0-missing handling
...
for i = 1:numel(zero_missing)
    nm = zero_missing(i);
    j = find(varNames == nm, 1);
    if ~isempty(j)
        col = X(:, j);
        col(col == 0) = NaN;
        X(:, j) = col;
