...
% Dense + larger threats (harder setting)
ThreatAreaPostion = [ ...
    58,  78;
    82,  90;
    98,  120;
...
ThreatAreaRadius = [12;14;10;12;14;12;10];

lb = [0*ones(1,NodesNumber), 0*ones(1,NodesNumber)];
ub = [200*ones(1,NodesNumber), 55*ones(1,NodesNumber)];
...
% store best position per run per method
...
% save to a NEW name to avoid overwriting previous scenario
save('/Users/zhengzikun/Desktop/UAV/uav_30runs_S3.mat', ...
     ...
     'best_pos');   
writetable(Summary, '/Users/zhengzikun/Desktop/UAV/uav_30runs_S3_summary.csv');

%% ============================================================
% Pick run per method (STRICT seeds from this run)
% Use MEDIAN run for each method (by final fitness)
% ============================================================
...
for k = 1:numel(methods)
    ...
    seed_used(k) = pick_idx;                  % because rng(r) with r = pick_idx
    bestPos_all(k,:) = squeeze(best_pos(pick_idx,k,:))';
end

disp(table(methods(:), seed_used, 'VariableNames', {'Method','MedianRunSeedUsedForPlot'}));
...
for i = 1:size(ThreatAreaRadius,1)
    ...
    Zc(2,:) = Zc(2,:) + 55;  % match ub z-range top (55)
...
title('Scenario S3: overlay of 8 best paths', 'FontName','Times New Roman');
set(gca,'FontName','Times New Roman');
xlim([0 200]); ylim([0 200]); zlim([0 100]);
grid on; box on; view(45,25);

print('-djpeg','-r600','/Users/zhengzikun/Desktop/UAV/uav_S3_overlay_8paths.jpg');
...
title('Scenario S3: overlay of 8 best paths (2D top view)', 'FontName','Times New Roman');
set(gca,'FontName','Times New Roman');
xlabel('x'); ylabel('y');

print('-djpeg','-r600','/Users/zhengzikun/Desktop/UAV/uav_S3_overlay_8paths_2D.jpg');
